...
if (!isset($_SESSION['email'])) {
    ...
    echo "<a href='LoginA.php'>Iniciar sesión</a>";
    exit();
}
...
$query = "SELECT p.nombre, p.apellido, p.telefono, p.direccion, p.email, u.administrador_id 
          FROM Administradores u JOIN Personas p ON u.persona_id = p.persona_id 
          WHERE p.email = '$email'";
...
if ($result->num_rows > 0) {
    ...
    $identificacion = $administrador['administrador_id'];
    $direccion = $administrador['direccion'];
} else {
    // Si no se encontró el administrador, redireccionar a la página de login
